...
const addToCreatedTasks = async (req, res) => {
  try {
      ...
      user.createdTasks.push({ text, description }); // Add description to the created tasks
      await user.save();
      res.status(200).send({
...
const toggleTaskController = async (req, res) => {
  ...
  const editTaskController = async (req, res) => {
    try {
        const { taskId } = req.params;
        const { text, description, taskType } = req.body; // Receive description from the request body
        const user = await userModel.findById(req.body.userId);
        ...
        if (task) {
            task.text = text;
            task.description = description; // Update description
            await user.save();
