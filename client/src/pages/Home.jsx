...
const Home = () => {
  ...
  const toggleTask = async (taskId, taskType) => {
    try {
      ...
      if (res.data.success) {
        ...
        setCompletedItems(userData.completedTasks);
        message.success(
          "Task toggled successfully between completed and incompleted tasks"
        );
      } else {
        message.error(res.data.message);
      }
  ...
  const deleteCompletedTask = async (taskId) => {
    try {
      const res = await axios.delete(
        `/api/v1/user/deleteCompletedTask/${taskId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const editTaskText = async (taskId, newText, newDescription, isCompleted) => {
    try {
      const taskType = isCompleted ? "completedTasks" : "createdTasks";
      const res = await axios.put(`/api/v1/user/editTask/${taskId}`,
        { text: newText, description:newDescription, taskType },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    <div className="outermost">
      ...
      <div className="container">
        ...
        <InputArea
          ...
          handleDescriptionChangeTrig={handleDescriptionChange}
          value={inputText}
        ...
        <ul>
          {items.map((task, index) => (
            <Item
              ...
              onEdit={(taskId, newText, newDescription, isCompleted) =>
                editTaskText(taskId, newText, newDescription, isCompleted)
              } 
            />
          ))}
